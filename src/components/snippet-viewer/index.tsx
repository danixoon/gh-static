...
const SnippetViewer: FC<Props> = (props: Props) => {
  const { snippet } = props;
  const [options, setOptions] = useState<(SnippetOption & { loaded?: boolean })[]>(() => snippet.options);
  ...
  useEffect(() => {
    ...
    snippet.options.forEach((o, i) => {
      codeToHtml(o.content, { lang: o.language, theme: 'github-dark' }).then((content) => {
        if (isCancelled) {
          return;
        }
        ...
        setOptions((prev) => {
          const updated = [...prev];
          updated[i] = { ...updated[i], content, loaded: true };
